...
interface IUserStore {
  ...
  isEdit: boolean;
  isShowMove: boolean;
  setIsShowMove: (isShowMove: boolean) => void;
  userId: number;
  userName: string;
  userList: IUser[];
  userListId: {
...
export default create<
  IUserStore,
  [["zustand/persist", { isEdit: boolean; isShowMove: boolean }]]
>(
  persist(
    (set, get) => ({
      isAuthLoading: true,
      ...
      setIsShowMove(isShowMove) {
        set({ isShowMove });
        const columns = useTrelloStore.getState().columns;
        if (columns.length > 0) {
          useTrelloStore.getState().setColumns(columns);
        }
      },
      async login() {
        try {
